...
class ODriveNode(object):
	...
	self.joint_state_topic      = rospy.get_param('~joint_state_topic', "joint_state")
	...
	if self.publish_joint_state:
	    ...
	    self.state_subscriber = rospy.Subscriber('motor_states/%s' % self.motor_id, MotorState, self.state_callback)
            # setup message
